...
export function SummaryTable({ legs, rows }: SummaryTableProps) {
  return (
    <div className="flex flex-col gap-2">
      <p className="px-1 text-xs font-medium tracking-wide text-slate-500 uppercase">{strings.summary.player}</p>
      {rows.map((row) => (
        <div key={row.playerId} className="rounded-lg border border-slate-200 p-3">
          <div className="flex items-center justify-between gap-2">
            <span className="font-medium">
              {row.totalNet >= 0 ? '🟢' : '🔴'} {row.playerName}
            </span>
            <span className={`text-lg font-semibold ${row.totalNet >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
